...
const colorChange = (color2) => {
  dispatch("colorChange", color2);
};
var PointerState = /* @__PURE__ */ ((PointerState2) => {
  PointerState2[PointerState2["Default"] = 0] = "Default";
  PointerState2[PointerState2["DownOnInner"] = 1] = "DownOnInner";
  PointerState2[PointerState2["DownOnOuter"] = 2] = "DownOnOuter";
  return PointerState2;
})(PointerState || {});
const MARGIN = (1 - 0.55) / 2;
let containerRef;
let canvasContainerRef;
let glState;
let pointerState = 0 /* Default */;
$:
...
function onDown(ev) {
  ...
  const marginX = bounds.width * MARGIN;
  const marginY = bounds.height * MARGIN;
  const isInner = isInclusive(x, marginX, bounds.width - marginX) && isInclusive(y, marginY, bounds.height - marginY);
  if (isInner) {
    pointerState = 1 /* DownOnInner */;
    signalInner(ev);
  } else {
    pointerState = 2 /* DownOnOuter */;
    signalOuter(ev);
  }
}
</script>
