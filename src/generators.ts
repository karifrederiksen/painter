...
export function always<a>(val: a): Generator<a> {
    return rng => [val, rng]
}
...
export function t2<a, b>(genA: Generator<a>, genB: Generator<b>): Generator<T2<a, b>> {
    return rng => t2_(genA, genB, rng)
}

function t2_<a, b>(genA: Generator<a>, genB: Generator<b>, rng0: Rng.Seed): GenResult<T2<a, b>> {
...
export function array<a>(length: number, gen: Generator<a>): Generator<Array<a>> {
    return rng => array_(length, gen, rng)
}

function array_<a>(length: number, gen: Generator<a>, rng: Rng.Seed): GenResult<Array<a>> {
...
export type ObjectProps<a> = { readonly [propName in keyof a]: Generator<a[propName]> }

export function object<a>(gens: ObjectProps<a>): Generator<a> {
...
export function map<a, b>(genA: Generator<a>, f: (valA: a) => b): Generator<b> {
    return rng => map_(genA, f, rng)
}

function map_<a, b>(genA: Generator<a>, f: (valA: a) => b, rng0: Rng.Seed): GenResult<b> {
    const [valA, rng1] = genA(rng0)
    return [f(valA), rng1]
}
...
export function andThen<a, b>(genA: Generator<a>, f: (val: a) => Generator<b>): Generator<b> {
...
export function lazy<a>(createGen: () => Generator<a>): Generator<a> {
    let cachedGen: Generator<a> | null = null
    return rng => {
        if (cachedGen === null) {
            cachedGen = createGen()
        }
