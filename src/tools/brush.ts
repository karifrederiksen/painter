...
export class Sender {
	...
	setColor = (color: Color.Hsluv) =>
		this.sendMessage(tagged("SetColor", color));
	setColorMode = (mode: ColorMode) =>
		this.sendMessage(tagged("SetColorMode", mode));
	setDelay = (delayMs: number) =>
		this.sendMessage(tagged("SetDelay", { delayMs }));
	setDiameter = (diameterPx: number) =>
		this.sendMessage(tagged("SetDiameter", { diameterPx }));
	setOpacity = (opacityPct: number) =>
		this.sendMessage(tagged("SetOpacity", { opacityPct }));
	setSoftness = (softnessPct: number) =>
		this.sendMessage(tagged("SetSoftness", { softnessPct }));
	setSpacing = (spacingPct: number) =>
		this.sendMessage(tagged("SetSpacing", { spacingPct }));
	setPressureAffectsOpacity = (setPressureAffectsOpacity: boolean) =>
		this.sendMessage(
			tagged("SetPressureAffectsOpacity", setPressureAffectsOpacity),
		);
...
export function update(state: Config, msg: Msg): Config {
	switch (msg.tag) {
		...
		case "SetColorMode":
			return {
				...state,
				colorMode: state.colorMode.focusf((x) => x === msg.val),
			};
		case "SetSpacing":
		...
		case "SetDelay":
			return {
				...state,
				delay: BrushDelay.delay(clamp(msg.val.delayMs, 0, 500)),
			};
		default: {
			msg satisfies never;
