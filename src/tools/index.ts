...
export class Sender {
	...
	constructor(private sendMessage: (msg: ToolMsg) => void) {
		this.brush = new Brush.Sender((msg) =>
			sendMessage(tagged("BrushMsg", msg)),
		);
		this.eraser = new Brush.Sender((msg) =>
			sendMessage(tagged("EraserMsg", msg)),
		);
		this.camera = new Camera.Sender((msg) =>
			sendMessage(tagged("CameraMsg", msg)),
		);
	}
...
export function onClick(
	...
	switch (ephemeral.tag) {
		case "Brush": {
			const [value, brushPoint] = Brush.onClick(
				tool.brush,
				tool.camera,
				pointer,
			);
...
export function onFrame(
	...
	switch (ephemeral.tag) {
		case "Brush": {
			const [value, brushPoints] = Brush.onFrame(
				tool.brush,
				ephemeral.val,
				currentTime,
			);
