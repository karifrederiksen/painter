...
test("(update) should produce state.end when delay is 0", (t) => {
	for (let time = 0; time < 10; time++) {
		t.expect(Delay.update(Delay.delay(0), state, time)[1]).deep.equals(
			state.end,
		);
	}
});
...
test("(update) should produce state.start when time is equal to startTime", (t) => {
	t.expect(Delay.update(Delay.delay(1), state, state.startTime)[1]).deep.equals(
		state.start,
	);
});
...
test("(update) should produce state.end when time is greater than or equal to (startTime + delay)", (t) => {
	for (let delay = 0; delay < 10; delay++) {
		for (let time = 0; time < 10; time++) {
			...
			t.expect(
				Delay.update(Delay.delay(delay), state, startTime)[1],
			).deep.equals(state.end);
		}
	}
});
...
test("(updateWithInput) works as expected", (t) => {
	...
	t.expect(
		Delay.updateWithInput(Delay.delay(0), state, 0, input)[1],
	).deep.equals(input);
	t.expect(
		Delay.updateWithInput(Delay.delay(0), state, 1, input)[1],
	).deep.equals(input);
	t.expect(
		Delay.updateWithInput(Delay.delay(1), state, 0, input)[1],
	).deep.equals(state.start);
	t.expect(
		Delay.updateWithInput(Delay.delay(1), state, 1, input)[1],
	).deep.equals(input);
});
