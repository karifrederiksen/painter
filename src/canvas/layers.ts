...
export class GroupLayer {
	...
	remove(
		selectedPath: Stack.NonEmpty<number>,
	): readonly [GroupLayer, Stack.Stack<number>] {
		const index = selectedPath.head;
		const selected = this.children[index];
		...
		if (selectedPath.tail.isNonEmpty()) {
			...
			const newSelectedPath =
				index === newIndex ? selectedPath : Stack.NonEmpty.of(newIndex);
...
export class State {
	...
	private newLayer(): State {
		const layers = this.layers.insert(
			this.selectedPath,
			LeafLayer.init(this.getNextLayerId()),
		);
	...
	private removeCurrent(): State {
		...
		const newIndex =
			newLayers.children.length <= oldIndex ? oldIndex : oldIndex - 1;
		return new State(
			this.getNextLayerId,
			newLayers,
			Stack.NonEmpty.of(newIndex),
		);
	}
