...
export type Effect =
	...
	| Tagged<
			"RenderFrame",
			{ brushPoints: readonly BrushPoint[]; config: Config }
	  >
...
export class Sender {
	...
	constructor(private sendMessage: (msg: CanvasMsg) => void) {
		...
		this.layer = new Layers.Sender((msg) =>
			sendMessage(tagged("LayersMsg", msg)),
		);
	}
	onFrame = (timeMs: number) => this.sendMessage(tagged("OnFrame", timeMs));
	onClick = (input: Input.PointerData) =>
		this.sendMessage(tagged("OnClick", input));
	onRelease = (input: Input.PointerData) =>
		this.sendMessage(tagged("OnRelease", input));
	...
	onKeyboard = (input: keymapping.KeyInput) =>
		this.sendMessage(tagged("OnKeyboard", input));
	randomizeTheme = () => this.sendMessage(tagged("RandomizeTheme"));
	toggleHighlightRenderBlocks = () =>
		this.sendMessage(tagged("ToggleHighlightRenderBlocks"));
		...
		keyboard: {
			layers: Stack.empty<keymapping.KeyBindLayer<CanvasMsg>>().cons({
				[createKey({ code: "KeyB" })]: {
					msg: tagged(
						"ToolMsg",
						tagged("SetToolMsg", "Brush") as Tools.ToolMsg,
					),
					passive: false,
				},
				[createKey({ code: "KeyE" })]: {
					msg: tagged(
						"ToolMsg",
						tagged("SetToolMsg", "Eraser") as Tools.ToolMsg,
					),
					passive: false,
				},
				[createKey({ code: "KeyZ" })]: {
				...
				[createKey({ code: "KeyR" })]: {
					msg: tagged(
						"ToolMsg",
						tagged("SetToolMsg", "Rotate") as Tools.ToolMsg,
					),
					passive: false,
				},
				[createKey({ code: "KeyP" })]: {
...
export function update(
	...
	switch (msg.tag) {
		...
		case "ToolMsg": {
			const nextConfig: Config = {
				...config,
				tool: Tools.update(config.tool, msg.val),
			};
			return [nextConfig, state, NOOP_EFFECT];
		}
		case "LayersMsg": {
			const nextConfig: Config = {
				...config,
				layers: config.layers.update(msg.val),
			};
			return [nextConfig, state, NOOP_EFFECT];
		}
		default: {
