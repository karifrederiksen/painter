...
export class SetOnce<a> {
  ...
  get value(): a {
    if (!this.__isSet) throw new Error("Attempted to get the value of a SetOnce before it was set");
    return this.__value as a;
  }
}
...
export function turn(turns: number, center: Vec2, point: Vec2): Vec2 {
  ...
  const x =
    Math.cos(radians) * (point.x - center.x) - Math.sin(radians) * (point.y - center.y) + center.x;
  const y =
    Math.sin(radians) * (point.x - center.x) + Math.cos(radians) * (point.y - center.y) + center.y;
  return new Vec2(x, y);
}
